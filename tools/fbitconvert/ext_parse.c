...
void ext0_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tZERO EXTENSION");
}
...
void ext1_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	if(TestFlag(flags, FLAG_IPV6_ADDR)){
		MSG_DEBUG(msg_str, "\tIPv6-SRC: hight:%lu low:%lu",*((uint64_t *) &data[*offset]), \
				*((uint64_t *) &data[(*offset)+2]));

		CONVERT_IPv6();

		MSG_DEBUG(msg_str, "\tIPv6-DST: hight:%lu low:%lu",*((uint64_t *) &data[*offset]), \
				*((uint64_t *) &data[(*offset)+2]));
		CONVERT_IPv6();

	} else {
		MSG_DEBUG(msg_str, "\tIPv4-SRC: %u", *((uint32_t *) &data[*offset]));
		CONVERT_32();

		MSG_DEBUG(msg_str, "\tIPv4-DST: %u", *((uint32_t *) &data[*offset]));
		CONVERT_32();
	}
}
...
void ext2_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	if(TestFlag(flags, FLAG_PKG_64)){
		MSG_DEBUG(msg_str, "\tPACKET COUNTER: %lu", *((uint64_t *) &data[*offset]));
		CONVERT_64();
	} else {
		MSG_DEBUG(msg_str, "\tPACKET COUNTER: %u", *((uint32_t *) &data[*offset]));
		//32b to 64b!
...
void ext3_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	if(TestFlag(flags, FLAG_BYTES_64)){
		MSG_DEBUG(msg_str, "\tBYTE COUNTER: %lu", *((uint64_t *) &data[*offset]));
		CONVERT_64();
	} else {
		MSG_DEBUG(msg_str, "\tBYTE COUNTER: %u", *((uint32_t *) &data[*offset]));
		//32b to 64b!
		*((uint64_t *) &(data_set->records[data_set->header.length])) =  htobe64((uint64_t)*((uint32_t *) &data[*offset]));
		data_set->header.length += 8;
		(*offset)+=1;
...
void ext4_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tINTERFACE RECORD INPUT: %hu (16b)", *((uint16_t *) &data[*offset]));
	MSG_DEBUG(msg_str, "\tINTERFACE RECORD OUTPUT: %hu (16b)", *(((uint16_t *) &data[*offset])+1));
	CONVERT_2x16();
...
void ext5_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tINTERFACE RECORD INPUT: %u (32b)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
	MSG_DEBUG(msg_str, "\tINTERFACE RECORD OUTPUT: %u (32b)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
}
...
void ext6_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tAS-SRC: %hu (16b)", *((uint16_t *) &data[*offset]));
	MSG_DEBUG(msg_str, "\tAS-DST: %hu (16b)", *(((uint16_t *) &data[*offset])+1));
	CONVERT_2x16();
...
void ext7_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tAS-SRC: %u (32b)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
	MSG_DEBUG(msg_str, "\tAS-DST: %u (32b)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
...
void ext8_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tDST-TOS: %hhu (8b)", *((uint8_t *) &data[*offset]));
	MSG_DEBUG(msg_str, "\tDIR: %hhu (8b)", *(((uint8_t *) &data[*offset])+1));
	MSG_DEBUG(msg_str, "\tSRC-MASK: %hhu (8b)", *(((uint8_t *) &data[*offset])+2));
	MSG_DEBUG(msg_str, "\tDST-MASK: %hhu (8b)", *(((uint8_t *) &data[*offset])+3));
...
void ext9_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tNEXT-HOP: %u (ipv4)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
...
void ext10_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	...
	MSG_DEBUG(msg_str, "\tNEXT-HOP: hight:%lu low:%lu (ipv6)",*((uint64_t *) &data[*offset]), \
...
void ext11_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tBGP-NEXT-HOP: %u (ipv4)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
...
void ext12_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tBGP-NEXT-HOP: hight:%lu low:%lu (ipv6)",*((uint64_t *) &data[*offset]), \
			*((uint64_t *) &data[(*offset)+8]));
	CONVERT_IPv6();
}
...
void ext13_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tVLAN-SRC: %hu (16b)", *((uint16_t *) &data[*offset]));
	MSG_DEBUG(msg_str, "\tVLAN-DST: %hu (16b)", *(((uint16_t *) &data[*offset])+1));
	CONVERT_2x16();
}
...
void ext14_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tOUT-PACKETS: %u (32b)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
...
void ext15_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tOUT-PACKETS: %lu (64b)", *((uint64_t *) &data[*offset]));
	CONVERT_64();
...
void ext16_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tOUT-BYTES: %u (32b)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
}
...
void ext17_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tOUT-BYTES: %lu (64b)", *((uint64_t *) &data[*offset]));
	CONVERT_64();
...
void ext18_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tAGGR-FLOWS: %u (32b)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
...
void ext19_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tAGGR-FLOWS: %lu (64b)", *((uint64_t *) &data[*offset]));
	CONVERT_64();
...
void ext20_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	uint64_t buf;
	MSG_DEBUG(msg_str, "\tIN-SRC-MAC: %lu (48b - 64 aling)", *((uint64_t *) &data[*offset]));
	buf = htobe64(data[*offset]);
	memcpy(&(data_set->records[data_set->header.length]), &buf, 6);
	data_set->header.length += 6;
	(*offset)+=2;
	MSG_DEBUG(msg_str, "\tOUT-DST-MAC: %lu (48b - 64 aling)", *((uint64_t *) &data[*offset]));
	buf = htobe64(data[*offset]);
	memcpy(&(data_set->records[data_set->header.length]), &buf, 6);
	data_set->header.length += 6;
	(*offset)+=2;
...
void ext21_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	uint64_t buf;
	MSG_DEBUG(msg_str, "\tIN-DST-MAC: %lu (48b - 64 aling)", *((uint64_t *) &data[*offset]));
	buf = htobe64(data[*offset]);
	memcpy(&(data_set->records[data_set->header.length]), &buf, 6);
	data_set->header.length += 6;
	(*offset)+=2;
	MSG_DEBUG(msg_str, "\tOUT-SRC-MAC: %lu (48b - 64 aling)", *((uint64_t *) &data[*offset]));
	buf = htobe64(data[*offset]);
	memcpy(&(data_set->records[data_set->header.length]), &buf, 6);
	data_set->header.length += 6;
	(*offset)+=2;
...
void ext23_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tROUTER-IP: %u (ipv4)", *((uint32_t *) &data[*offset]));
	CONVERT_32();
}
...
void ext24_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tROUTER-IP: hight:%lu low:%lu (ipv6)",*((uint64_t *) &data[*offset]), \
			*((uint64_t *) &data[(*offset)+8]));
	CONVERT_IPv6();
...
void ext25_parse(uint32_t *data, int *offset, uint8_t flags, struct ipfix_data_set *data_set){
	MSG_DEBUG(msg_str, "\tROUTER-ID-FILL: %hu ", *((uint16_t *) &data[*offset]));
	*((uint16_t *) &(data_set->records[data_set->header.length])) = htons(*((uint16_t *) &data[*offset])+1);
	data_set->header.length += 2;
	MSG_DEBUG(msg_str, "\tROUTER-ID-ENGINE-TYPE: %hhu ", *(((uint8_t *) &data[*offset])+2));
	MSG_DEBUG(msg_str, "\tROUTER-ID-ENGINE-ID: %hhu ", *(((uint8_t *) &data[*offset])+3));
