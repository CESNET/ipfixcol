...
{
	if (err) {
		MSG_ERROR(msg_module, "Message delivery failed: %s", rd_kafka_err2str(err));
	}
}
...
Kafka::Kafka(const pugi::xpath_node &config)
{
	...
	if (!_rk) {
		throw std::runtime_error(std::string("Failed to create new producer: ") + errstr);
	}
	...
	if (!_rkt) {
		...
		throw std::runtime_error(std::string("Failed to create topic: ") + rd_kafka_err2str(rd_kafka_errno2err(errno)));
	}
...
{
	while (rd_kafka_produce(_rkt, _current_partition++ % _partitions, RD_KAFKA_MSG_F_COPY, (void *) record.c_str(), record.length(), NULL, 0, NULL) != 0) {
		// Wait before sending
		switch (errno) {
		case ENOBUFS:
			MSG_WARNING(msg_module, "maximum number of outstanding messages (%u) has been reached: 'queue.buffering.max.messages'", rd_kafka_outq_len(_rk));
			usleep(200000);
			rd_kafka_poll(_rk, 0);
			break;
		case EMSGSIZE:
			MSG_ERROR(msg_module, "Message is larged than configured max size: 'messages.max.bytes'");
			return;
		case ESRCH: 
			throw std::runtime_error("Requested 'partition' is unknown in the Kafka cluster.");
			break;
		case ENOENT:
			throw std::runtime_error("Topic is unknown in the Kafka cluster.");
			break;
		default:
			MSG_ERROR(msg_module, "Unknown error while producing a message to Kafka");
			break;
		}
	}
